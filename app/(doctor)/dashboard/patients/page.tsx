        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <Input 
              ...
              className="pl-9 bg-[var(--background)] h-10"
              value={searchTerm}
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
            {filterButtons.map(btn => (
              <Button 
                ...
                size="sm"
                className={cn("h-9 px-3 text-xs whitespace-nowrap", activeFilter === btn.value && btn.value === 'danger' && "bg-[var(--danger-bg)] text-[var(--status-danger)] hover:bg-[var(--danger-bg)]/80")}
        ...
        <div className="flex flex-col divide-y divide-[var(--border)]">
          {filteredPatients.length === 0 ? (
            ...
            filteredPatients.map((patient) => (
              <Link key={patient.id} href={`/dashboard/patients/${patient.id}`} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 hover:bg-[var(--surface-2)] transition-colors gap-4 cursor-pointer">
